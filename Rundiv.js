...
function Rundiv(container, options) {
  ...
  options = options || {};
  var index = parseInt(options.startSlide, 10)-1 || 0;
  var speed = options.speed || 300;
  options.continuous = typeof options.continuous !== 'undefine' ? options.continuous : true;
  ...
  function setup() {
    ...
    while(pos--) {
      ...
      slide.setAttribute('data-index', pos+1);
  ...
  function mark(obj){
    ...
    var page = tmp.length;
    while(page--){
      tmp[page].setAttribute('data-index',page+1);
    }
  ...
  var events = {
    ...
    move: function(event) {
      ...
      // determine direction of Rundiv (true:right, false:left)
...
if ( window.jQuery || window.Zepto ) {
  (function($) {
    $.fn.Rundiv = function(params) {
      return this.each(function() {
        $(this).data('Rundiv', new Rundiv($(this)[0], params));
      });
    }
